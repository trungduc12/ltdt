...
namespace Prims_Algorithm
{
    public partial class Form1 : Form
    {
        ...
        {
            listInTreeVertex = new List<Vertex>();//danh sách các đỉnh trong cây khung nhỏ nhất
            // Copy list vertex
            copiedListVertex = listVertex.ToList();//tạo danh sách đỉnh
            copiedListEdge = listEdge.ToList();//tạo danh sách cạnh

            foreach (Vertex vertex in listVertex)
            {
                // set key to infinite
                vertex.Key = 999999;//giá trị
                // set parent to null
                vertex.Parent = null;
            }

            // Add root vertex to intree vertex list
            Vertex rootVertex = new Vertex();//đỉnh gốc
            if (cbRootVertex.SelectedIndex != -1)
            {
                rootVertex = listVertex[cbRootVertex.SelectedIndex];// lấy đỉnh gốc từ form
            }
            ...
            // 
            while (listInTreeVertex.Count < listVertex.Count)//số đỉnh trong cây khung=tất cả các đỉnh
            {
        ...
        private void btnAddEdge_Click(object sender, EventArgs e)
        {
           if (tbDistance.Text.Trim().Length == 0)
            {
                MessageBox.Show("Vui lòng nhập trọng số");
            }
            else
            {
            ...
            tbDistance.Text = cbFromVertex.Text = cbToVertex.Text = String.Empty;
        }
        ...
        private void btnThemDinh_Click(object sender, EventArgs e)
        {
            ...
            tbVertexName.Text = string.Empty;
        }
        ...
        private void btnDeleteVertex_Click(object sender, EventArgs e)
        {
            if (lbVertex.SelectedIndex != -1)
            {
                ...
                {
                    MessageBox.Show(" Có một cạnh liên quan đến đỉnh này !");
                }
            }
        }
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                while ((line = file.ReadLine()) != null)
                {
                    ...
                    else if (line[0] == '-')
                    {
                        ...
                        {
                            MessageBox.Show("Có lỗi trong tệp văn bản !" + counter);
                        }
                        else
                        {
                            int distance = -1;
                            ...
                            {
                                MessageBox.Show("Có lỗi về khoảng cách trong file văn bản " + counter);
                                throw;
                            }
                            finally
                            {
